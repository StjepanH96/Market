...
const fetchInitializeProductsAction =
  ({ page, limit }: FetchProductsParams) =>
  async (dispatch: any) => {
...
const fetchInitiliazeProductByIdAction =
  (productId: string | string[]): AppThunk =>
  async (dispatch) => {
...
const fetchInitiliazeSortedProductsByPrice =
  (sortBy: string, order: string): AppThunk =>
  async (dispatch) => {
    dispatch(setLoading(true));
...
const fetchProductsByCategoryAction =
  (categoryName: string | string[] | undefined): AppThunk =>
  async (dispatch) => {
    dispatch(setLoading(true));
